...
void main()
{
	...
	v_worldToTangentMatrix = inverse(TangentToWorldMatrix);
	...
	float noise = snoise( vec3(v_textureCoords, v_worldPosition.z) - vec3( time, 0.0,0.0 ) );
	float noise_2 = noise + 0.5 * snoise(vec3(v_textureCoords, v_worldPosition.z) * 2.0 - (vec3( time, 0.0,0.0 ) + vec3(0.0, 0.0, 1.4))); 
	vec3 noiseVec3 = vec3( noise_2 * 0.7, noise_2 * 0.7, noise_2 * 0.7);
	vec3 newPosition = v_worldPosition + ( noiseVec3 * a_vertexNormal * 0.2 );
	if (length(newPosition) < length(v_worldPosition)) 
		newPosition = v_worldPosition;
	v_screenPosition = u_WorldToScreenMatrix * u_transformMatrix * u_scaleMatrix * vec4( 0.1 + newPosition , 1.0 );
